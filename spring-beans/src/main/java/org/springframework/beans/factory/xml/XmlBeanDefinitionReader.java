...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * 从指定的入Resource资源载入Xml文件 保证Reader可以在打开I/O流后得到XML文件对象
	 *
	 ...
	 * <p>
	 * 该方法是具体的读取过程 从特定的XML文件实际载入BeanDefinition
	 *
	 * @param inputSource the SAX InputSource to read from
	 * @param resource    the resource descriptor for the XML file
	 * @return the number of bean definitions found
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			/**
			 * 1.获取XML文件的Document对象 解析过程是由DocumentLoader完成的 这个DocumentLoader是DefaultDocumentLoader
			 */
			Document doc = doLoadDocument(inputSource, resource);
			/**
			 * 2.启动的是对BeanDefinition解析的详细过程 这个解析会用到Spring的Bean的配置规则
			 */
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		/**
		 * 在得到BeanDefinitionDocumentReader之后 为具体的Spring Bean的解析过程准备好了数据
		 * 具体的解析过程在这个registerBeanDefinitions()方法中完成
		 */
